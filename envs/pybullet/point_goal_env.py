...
class BioloidEnvPointGoal(gym.Env):
    ...
    def __init__(
        ...
        walker_path: str = os.path.join("models", "pybullet", "sac", "Walker.pth"),
        turn_left_path: str = os.path.join("models", "pybullet", "sac", "Left_Turner.pth"),
        turn_right_path: str = os.path.join("models", "pybullet", "sac", "Right_Turner.pth"),
        # Control parameters
        command_duration_steps: int = 30, # How many physics steps to execute a command for
        goal_distance_threshold: float = 0.2, # meters
        urdf_path: str = os.path.join("assets", "pybullet", "Bioloid_Quadruped_Model.urdf"),
    ):
        ...
        self.base_env = BioloidAntLikeEnv(render_mode=render_mode, max_steps=max_steps * command_duration_steps, urdf_path=urdf_path)
