...
class BioloidMujocoTurnLeftEnv(gym.Env):
    """
    MuJoCo environment for the Bioloid Quadruped optimized for Turning Left.
    Matches 'envs/turn_left_env.py' exactly.
    """
    def __init__(
        self,
    ...
    def _get_obs(self):
        ...
        # Quaternion: MuJoCo (w, x, y, z) -> PyBullet (x, y, z, w)
    ...
    def step(self, action):
        ...
        # Base state for rewards
        base_pos = self.data.qpos[:3]
        lin_vel = self.data.qvel[:3]
        ang_vel = self.data.qvel[3:6]
        
        # 1. Turn Reward (Left is positive Z-ang velocity)
