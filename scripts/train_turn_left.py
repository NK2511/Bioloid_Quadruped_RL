...
# Add project root and the environments directory to the Python path
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.append(PROJECT_ROOT) # For sac_agent, replay_memory
# sys.path.append(os.path.join(PROJECT_ROOT, 'envs'))
from envs.turn_left_env import BioloidAntLikeEnvTurnLeftOnly
...
def save_checkpoint(agent, directory: str, episode: int, total_steps: int, updates: int):
    ...
    path = os.path.join(directory, f"turner_left_checkpoint_ep{episode}.pth")
...
def parse_args():
    p = argparse.ArgumentParser(description="Train a 'Turner' brain to turn LEFT from a standing start.")
    p.add_argument("--gui", action="store_true", help="Run with PyBullet GUI to watch.")
    p.add_argument("--save-dir", type=str, default="../bioloid_checkpoints/left_turner_checkpoints")
    p.add_argument("--log-dir", type=str, default="../runs/left_turner")
    p.add_argument("--checkpoint-interval", type=int, default=100)
    p.add_argument("--seed", type=int, default=7) # Different seed from right turner
...
def main():
    ...
    env = BioloidAntLikeEnvTurnLeftOnly(render_mode="GUI" if args.gui else "DIRECT")
    ...
    print("\n--- Standalone LEFT Turner Training ---")
    print(f"Agent will be trained to turn LEFT.")
