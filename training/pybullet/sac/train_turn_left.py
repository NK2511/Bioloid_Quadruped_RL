...
from envs.pybullet.turn_left_env import BioloidAntLikeEnvTurnLeftOnly
...
def load_full_resume(agent, resume_full_path: str, device: torch.device) -> Dict[str, int]:
    ...
    if resume_full_path:
        if not os.path.isfile(resume_full_path):
            raise FileNotFoundError(f"[Resume Error] Specified checkpoint not found: {resume_full_path}")
        
    if not resume_full_path:
        return info
...
def parse_args():
    ...
    p.add_argument("--save-dir", type=str, default="journal_paper/turn_left/checkpoints")
    p.add_argument("--log-dir", type=str, default="journal_paper/turn_left/runs")
    p.add_argument("--checkpoint-interval", type=int, default=50, help="Every N episodes, save a full checkpoint (0=disabled)")
    p.add_argument("--checkpoint-dir", type=str, default="journal_paper/turn_left/checkpoints", help="Directory to store full checkpoints (defaults to save-dir)")
...
def main():
    ...
    env = BioloidAntLikeEnvTurnLeftOnly(render_mode="DIRECT")
    ...
    # Correction for older checkpoints that didn't save total_steps
    if start_episode > 0 and start_total_steps == 0:
        print(f"[Resume Fix] Checkpoint has episode {start_episode} but 0 steps. Estimating steps...")
        # Assume ~1000 steps per episode for the sake of the counter/graph
        start_total_steps = start_episode * 1000 
        print(f"[Resume Fix] Estimated start_total_steps = {start_total_steps}")
    ...
    scores, avg_scores = sac_train(
        ...
        checkpoint_dir=checkpoint_dir  # Pass checkpoint_dir to save properly in finally block
    )


def sac_train(env, agent, memory, batch_size, start_steps, num_episodes, max_steps, callbacks: List[Callback], gui_all=False, gui_every=0, eval_interval=0, eval_episodes=5, save_dir="dir_bioloid_ant_like", checkpoint_interval=0, resume_episode=0, resume_total_steps=0, resume_updates=0, checkpoint_dir=None):
    ...
    try:
        ...
        print("Saving training data before exit...")
        # Save raw training data for plotting
        if checkpoint_dir:
             np.savez(os.path.join(checkpoint_dir, "training_data.npz"), scores=scores_array, avg_scores=avg_scores_array)
             print(f"Saved training data to {os.path.join(checkpoint_dir, 'training_data.npz')}")
...
def evaluate_policy(agent, episodes=5):
    env_eval = BioloidAntLikeEnvTurnLeftOnly(render_mode="DIRECT")
    total = 0.0
