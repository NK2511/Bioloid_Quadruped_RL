...
from envs.pybullet.point_goal_env import BioloidEnvPointGoal
from brains.sac.sac_agent_discrete import SACDiscreteAgent # Use the new discrete agent
...
def load_resume(agent, resume_path: str, device: torch.device) -> dict:
    ...
    print(f"[SUCCESS] Resuming training from: {os.path.basename(resume_path)} at episode {info['episode']}")
    return info
...
def parse_args():
    ...
    p.add_argument("--save-dir", type=str, default="journal_paper/point_goal/checkpoints", help="Directory to save the new Point-Goal model.")
    p.add_argument("--log-dir", type=str, default="journal_paper/point_goal/runs", help="Directory for TensorBoard logs.")
    ...
    p.add_argument("--walker-path", type=str, default=r"models\Walker.pth", help="Path to the walker expert checkpoint.")
    p.add_argument("--turn-left-path", type=str, default=r"models\Left_Turner.pth", help="Path to the turn-left expert checkpoint.")
    p.add_argument("--turn-right-path", type=str, default=r"models\Right_Turner.pth", help="Path to the turn-right expert checkpoint.")
    return p.parse_args()
...
def main():
    ...
    env = BioloidEnvPointGoal(
        render_mode="GUI" if args.gui else "DIRECT",
    ...
    try:
        for i_episode in range(start_episode, args.num_episodes):
            ...
            if info.get("goal_reached"):
                print(f"[REACHED] Waypoint reached in {episode_steps} steps!")
        ...
        np.savez(os.path.join(args.save_dir, "training_data.npz"), scores=scores_array, avg_scores=avg_scores_array)
        print(f"Saved training data to {os.path.join(args.save_dir, 'training_data.npz')}")
